...
public class test105前序中序恢复树 {
   ...
   public TreeNode buildTree(int[] pre, int preStart, int preEnd, int[] in, int inStart, int inEnd){
       ...
       int rootIndex = 0;
       for(int i = inStart; i <= inEnd; i++){   //找到root在中序中的序号
            if(in[i] == rootVal){
                rootIndex = i;
                break;
            }
        ...
        int len = rootIndex - inStart;   //中序前半部分的长度
        TreeNode root = new TreeNode(rootVal);
        root.left = buildTree(pre, preStart+1, preStart+len, in, inStart, rootIndex-1); //前序第一个元素为root  之后非诚2部分       中序从root开始分成两部分   左子树为前半部分
        root.right = buildTree(pre, preStart+len+1, preEnd, in, rootIndex+1, inEnd); //前序第一个元素为root  之后非诚2部分       中序从root开始分成两部分   右子树为后半部分
